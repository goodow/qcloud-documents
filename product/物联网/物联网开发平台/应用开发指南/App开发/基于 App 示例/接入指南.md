...
## 接入应用端API   
   ...
   **注意: 登录前所使用的API （https://iot.cloud.tencent.com/api/exploreropen/appapi），不建议在设备端调用，需要替换为自建的后台服务，以避免密钥的泄漏。**
     API（https://iot.cloud.tencent.com/api/studioapp）为公版APP所使用的，OEM的App需要替换为应用端的API（https://iot.cloud.tencent.com/api/exploreropen/appapi）
   ...
   **登录前所使用的 API URL** 在 **self.baseUrl** 配置，**请务必替换成自建的后台服务。**
     ```
     - (void)setEnvironment {
   ...
   **登录前所使用的 API URL** 在 **HOST + EXPLORER_API** 配置，**请务必替换成自建的后台服务。**
     ```
     /**
      * 接口请求文件
      */
